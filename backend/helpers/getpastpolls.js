...
export const getpastpolls = async(req, getfull)=>{
    try {
        let data
        if(getfull){
            console.log("instance: ", req.query.instance)
             data = await pastpollsmodel.findOne({qid: req.query.qid, instance: req.query.instance})
             console.log("view past poll: ", data)
        }
        else { data = await pastpollsmodel.find({author: req.query.author},{qid:1, qname:1, instance:1, updatedAt:1})}
        return data
    } catch (error) {
