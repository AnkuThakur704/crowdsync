...
const pastquizzes = () => {
    const [allpastquizzes, setallpastquizzes] = useState([])
    const {userdata}  =  useAuth()
    const getpastquizzes= async()=>{
        let data = await fetch(`${backend_url}/routes/getpastquizzes?author=${userdata.email}`);
        data = await data.json();
        console.log(data)
        setallpastquizzes(data.pastquizzes)
    }
    useEffect(() => {
      getpastquizzes()
    }, [userdata])
    ...
    useEffect(() => {
      console.log("all past quizzes updated")
    }, [allpastquizzes])
  ...
  <div className="max-w-7xl mx-auto">
    ...
    <div className="mb-12">
      ...
      <p className="text-gray-400 mt-3 text-lg">
        View previously hosted quizzes and their leaderboards.
      </p>
    </div>

    {allpastquizzes.length != 0 ? (
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">

        {allpastquizzes.map((quiz, key) => (
          <div
            key={key}
          ...
          >
            ...
            <div className="space-y-4 text-sm">
              ...
              <div className="flex justify-between items-center pt-2">
                ...
                <span className="text-gray-600">
                  {givelastupdated(new Date(quiz.updatedAt))}
                </span>
            ...
            <Link to={`/viewpastquiz?qid=${quiz.qid}&instance=${quiz.instance}`}>
              <button
